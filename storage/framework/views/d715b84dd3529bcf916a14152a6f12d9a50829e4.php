...
<div class="w-full max-w-none py-3 flex flex-col min-h-0 max-h-[calc(100dvh-var(--topbar-h,4rem)-1.25rem)] overflow-hidden">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3 shrink-0">
    <div>
      ...
      <p class="text-sm text-gray-500 mt-0.5">Ringkasan permasalahan & root cause — filter per bulan.</p>
    </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:items-stretch flex-1 min-h-0 overflow-hidden">
    ...
    <div class="lg:col-span-1 col-span-1 min-h-0 flex">
      <div class="bg-white p-4 rounded-2xl shadow-sm ring-1 ring-gray-100 h-full w-full min-h-0 flex flex-col overflow-hidden">
        ...
        <div id="kpiGrid" class="grid grid-cols-2 gap-2 shrink-0 mt-2">
          <div class="p-2.5 bg-gray-50 rounded-lg">
            <div class="text-[11px] text-gray-500 leading-tight">Total Tiket</div>
            <div id="kpiTotal" class="text-base font-semibold text-gray-800 mt-0.5">—</div>
          </div>
          <div class="p-2.5 bg-gray-50 rounded-lg">
            <div class="text-[11px] text-gray-500 leading-tight">Tiket Terbuka</div>
            <div id="kpiOpen" class="text-base font-semibold text-gray-800 mt-0.5">—</div>
          </div>
          <div class="p-2.5 bg-gray-50 rounded-lg">
            <div class="text-[11px] text-gray-500 leading-tight">Tiket Selesai</div>
            <div id="kpiClosed" class="text-base font-semibold text-gray-800 mt-0.5">—</div>
          </div>
          <div class="p-2.5 bg-gray-50 rounded-lg">
            <div class="text-[11px] text-gray-500 leading-tight">Rata-rata Penyelesaian</div>
            <div id="kpiAvg" class="text-base font-semibold text-gray-800 mt-0.5">—</div>
          </div>
          <div class="p-2.5 bg-gray-50 rounded-lg col-span-2">
            <div class="text-[11px] text-gray-500 leading-tight">Respon Time TI</div>
            <div id="kpiResponse" class="text-base font-semibold text-gray-800 mt-0.5">—</div>
          </div>
        </div>

        <div class="mt-2 border-t border-gray-100 pt-2 flex flex-col flex-1 min-h-0">
          ...
          <div class="flex flex-1 min-h-0 flex-col items-center justify-center gap-1.5">
            <div class="relative h-36 w-36 shrink-0 sm:h-40 sm:w-40">
              <canvas id="chartStatus" class="absolute inset-0 h-full w-full"></canvas>
            </div>
            <ul id="chartStatusLegend" class="shrink-0 flex w-full max-w-full flex-row flex-wrap items-center justify-center gap-x-2 gap-y-0.5 text-[10px] leading-tight" role="list" aria-label="Legenda status tiket"></ul>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="lg:col-span-2 md:col-span-1 col-span-1 min-h-0 flex">
      <div class="bg-white p-4 rounded-2xl shadow-sm ring-1 ring-gray-100 w-full flex flex-col gap-3 min-h-0 h-full overflow-hidden">
        <div class="shrink-0">
          ...
          <div class="relative h-36 w-full sm:h-40">
            <canvas id="chartKategori" class="absolute inset-0 h-full w-full"></canvas>
          </div>
        </div>

        <div class="flex-1 min-h-[14rem] border-t border-gray-100 pt-3 grid grid-cols-1 gap-4 lg:grid-cols-2 lg:min-h-[16rem]">
          <div class="min-w-0 flex flex-col min-h-[12rem]">
            <div class="flex items-center justify-between gap-2 mb-1.5 shrink-0">
            ...
            <div class="relative flex-1 min-h-[11rem] w-full">
...
<script>
  ...
  /** Warna solid per status (tanpa gradasi). */
  function statusDonutColor(label, idx) {
    const key = String(label || '');
    const map = {
      OPEN: '#f87171',
      ON_PROGRESS: '#fbbf24',
      TAKEN: '#fb923c',
      ESKALASI_VENDOR: '#e879f9',
      VENDOR_RESOLVED: '#818cf8',
      CLOSED: '#34d399',
    };
    if (map[key]) return map[key];
    const fallbacks = ['#6366f1', '#0ea5e9', '#a855f7', '#64748b'];
    return fallbacks[idx % fallbacks.length];
  }

  function statusDonutSolidFills(labels) {
    return (labels || []).map((lbl, i) => statusDonutColor(lbl, i));
  }
  ...
  function renderChartStatusLegend(labels, data) {
    ...
    labels.forEach((raw, i) => {
      ...
      const swatchColor = statusDonutColor(raw, i);
      const li = document.createElement('li');
      li.className = 'inline-flex shrink-0 items-center gap-1';
      li.setAttribute('role', 'listitem');

      const sw = document.createElement('span');
      sw.className = 'h-3 w-3 shrink-0 rounded-md ring-1 ring-black/10 shadow-sm';
      ...
      num.className = 'shrink-0 tabular-nums text-[10px] font-semibold text-gray-500';
  ...
  async function fetchStats(from = '', to = '') {
    try {
      ...
      requestAnimationFrame(() => {
        try {
          ['chartStatus', 'chartKategori', 'chartTopUser', 'chartRootCause'].forEach((id) => {
