<?php $__env->startSection('title','Tiket Saya (IT)'); ?>
...
<form method="GET" class="space-y-3 md:space-y-0 md:flex md:flex-wrap md:items-end md:gap-2" id="filter-form">
      ...
      <div class="w-full md:w-auto flex flex-wrap gap-2">
        <button type="submit" class="w-full md:w-auto hd-btn-primary px-4 py-2">Filter</button>
  ...
  <div class="hidden md:block overflow-x-auto">
    <table class="min-w-full text-sm">
      ...
      <tbody class="divide-y divide-gray-100">
        ...
        <tr class="<?php echo e($i % 2 === 0 ? 'bg-white' : 'bg-gray-50'); ?> hover:bg-hd-50/60">
          <td class="py-3 px-4 text-gray-500"><?php echo e($tickets->firstItem()+$i); ?></td>
          <td class="py-3 px-4 font-medium"><a href="<?php echo e(route('ticket.show',$t->id)); ?>" class="hd-link"><?php echo e($t->nomor_tiket); ?></a></td>
          <td class="py-3 px-4"><?php echo e($t->kategori); ?></td>
          <td class="py-3 px-4"><?php echo e($t->user->name ?? '-'); ?></td>
          <td class="py-3 px-4">
            <?php $badge = $t->status==='OPEN'?'bg-gray-100 text-gray-700 ring-gray-200':($t->status==='ON_PROGRESS'?'bg-amber-100 text-amber-800 ring-amber-200':'bg-emerald-100 text-emerald-800 ring-emerald-200'); ?>
          ...
          <td class="py-3 px-4 space-x-1">
            <a href="<?php echo e(route('ticket.show',$t->id)); ?>" class="hd-btn-detail">Detail</a>
            <?php if($t->status==='ON_PROGRESS'): ?>
              ...
              <form method="POST" class="inline" action="<?php echo e(route('it.ticket.close',$t->id)); ?>"><?php echo csrf_field(); ?>
                <button class="rounded-lg bg-emerald-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-emerald-700">Tutup</button>
              </form>
            <?php endif; ?>
          </td>
        </tr>
    ...
    <?php $__empty_1 = true; $__currentLoopData = $tickets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <div class="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div class="flex items-start justify-between gap-3">
          <div>
            <a href="<?php echo e(route('ticket.show',$t->id)); ?>" class="font-semibold hd-link"><?php echo e($t->nomor_tiket); ?></a>
            <div class="mt-1 text-xs text-gray-500">Dibuat: <?php echo e($t->created_at->format('d M Y H:i')); ?></div>
          </div>
          <?php $badge = $t->status==='OPEN'?'bg-gray-100 text-gray-700 ring-gray-200':($t->status==='ON_PROGRESS'?'bg-amber-100 text-amber-800 ring-amber-200':'bg-emerald-100 text-emerald-800 ring-emerald-200'); ?>
          <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ring-1 <?php echo e($badge); ?>"><?php echo e($t->status); ?></span>
        </div>
        <div class="mt-3 grid grid-cols-2 gap-2 text-sm">
          <div class="text-gray-500">Kategori</div><div class="font-medium"><?php echo e($t->kategori); ?></div>
          <div class="text-gray-500">Pembuat</div><div class="font-medium"><?php echo e($t->user->name ?? '-'); ?></div>
        </div>
        <div class="mt-3 flex flex-wrap gap-2">
          <a href="<?php echo e(route('ticket.show',$t->id)); ?>" class="hd-btn-detail px-3 py-2">Detail</a>
          <?php if($t->status==='ON_PROGRESS'): ?>
            <form method="POST" action="<?php echo e(route('it.ticket.release',$t->id)); ?>"><?php echo csrf_field(); ?>
              <button class="hd-btn-lepas px-3 py-2">Lepas</button>
            </form>
