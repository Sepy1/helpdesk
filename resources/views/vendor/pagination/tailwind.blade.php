                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="hd-pagination-active">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-hd-600 bg-white border border-gray-300 leading-5 hover:bg-hd-50 focus:z-10 focus:outline-none focus:ring ring-hd-200 focus:border-hd-300 active:bg-hd-50 active:text-hd-700 transition ease-in-out duration-150" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
