    ...
    <table class="min-w-full text-sm">
      <thead class="text-left bg-gray-50 text-gray-600">
        <tr>
          ...
          <th class="py-3 px-4">IT Handler</th>
      ...
      <tbody class="divide-y divide-gray-100">
            ...
            <td class="py-3 px-4">
              @if($t->status=='OPEN' || ($t->status=='ON_PROGRESS' && optional($t->it)->id === auth()->id()))
                <form method="POST" action="{{ route('it.ticket.take',$t->id) }}">
                  @csrf
                  <button class="inline-flex items-center rounded-lg bg-indigo-600 px-3 py-1.5 text-white hover:bg-indigo-700">
                    Ambil Alih
                  </button>
                </form>
              @else
                <button class="inline-flex items-center rounded-lg bg-gray-100 px-3 py-1.5 text-gray-500 cursor-not-allowed" disabled>
                  Sudah Diambil
                </button>
              @endif
            </td>
        ...
        @empty
          <tr><td colspan="7" class="py-6 px-4 text-center text-gray-500">Belum ada tiket.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
