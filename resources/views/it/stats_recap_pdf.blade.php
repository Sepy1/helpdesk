...
<body>
  <div class="header">
    ...
    <div class="meta">
      Periode: {{ $dateFrom ?: 'Semua' }} &mdash; {{ $dateTo ?: 'Semua' }}<br>
      Kantor pembuat: {{ $officeLabel }}<br>
    ...
    <tbody>
      @foreach($statusRows as $row)
        <tr>
          ...
          <td class="number">{{ number_format($row['percentage'], 2, ',', '.') }}%</td>
        </tr>
      @endforeach
    </tbody>
    <tfoot>
      <tr>
        <td>Total</td>
        <td class="number">{{ number_format($total, 0, ',', '.') }}</td>
        <td class="number">{{ $total > 0 ? '100,00%' : '0,00%' }}</td>
      </tr>
    </tfoot>
  </table>
  ...
  <table class="office-table">
    ...
    <tbody>
      @forelse($officeRows as $office)
        <tr>
          <td>{{ $office['office'] }}</td>
          <td class="number">{{ number_format($office['total'], 0, ',', '.') }}</td>
          <td>{{ $office['roots'] ?: 'Tidak tersedia' }}</td>
          <td>{{ $office['categories'] ?: 'Tidak tersedia' }}</td>
          <td>{{ $office['subcategories'] ?: 'Tidak tersedia' }}</td>
        </tr>
      @empty
        <tr>
