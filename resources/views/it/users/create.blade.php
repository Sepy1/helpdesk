  ...
  <div class="space-y-4">
    <section class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 class="mt-2 text-2xl font-semibold tracking-normal text-slate-900 sm:text-3xl">Tambah User</h1>
    ...
    <section class="rounded-lg border border-slate-200 bg-white shadow-sm">
      <form method="POST" action="{{ route('it.users.store') }}" class="p-4 sm:p-5">
        @csrf
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label for="username" class="{{ $label }}">Username</label>
            <input id="username" type="text" name="username" value="{{ old('username') }}" class="{{ $input }}" required>
          ...
          <div>
            ...
            <select id="role" name="role" class="{{ $input }}" required>
              @foreach($roles as $role)
                <option value="{{ $role }}" @selected(old('role', 'IT') === $role)>{{ $role }}</option>
              @endforeach
            </select>
          ...
          <div>
            ...
            <select id="kode_kantor" name="kode_kantor" class="{{ $input }}">
              ...
              @foreach($kodeKantors as $office)
                <option value="{{ $office->kode }}" @selected(old('kode_kantor') === $office->kode)>{{ $office->kode }} - {{ $office->nama_kantor }}</option>
              @endforeach
            </select>
            <p class="mt-1 text-xs text-slate-500">Opsional, terutama untuk user cabang.</p>
        ...
        <div class="mt-5 flex flex-wrap justify-end gap-2 border-t border-slate-100 pt-4">
          ...
          <button type="submit" class="{{ $btnPrimary }}">Simpan User</button>
        </div>
      </form>
    </section>
  </div>
