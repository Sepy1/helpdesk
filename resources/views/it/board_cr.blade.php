...
<div class="min-h-[calc(100vh-9rem)] rounded-3xl border border-white/10 bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 p-4 sm:p-5 text-slate-100 shadow-2xl shadow-indigo-950/20">
  <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
    <div>
      ...
      <p class="mt-2 text-xs text-slate-400">
        Endpoint: <span class="font-mono text-slate-200">{{ config('services.extern_cr.base_url') }}{{ config('services.extern_cr.dashboard_path') }}</span>
      </p>
    </div>
    ...
    <div class="flex gap-2">
      ...
      <a href="{{ route('it.dashboard') }}" class="rounded-xl bg-sky-500 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-400">
        Kembali ke Tiket
      </a>
    </div>
  </div>
  ...
  <div class="mt-5 grid gap-4 xl:grid-cols-5">
    @foreach($columns as $column)
      <section class="board-column rounded-2xl border border-white/10 bg-white/5 p-3" data-status="{{ $column['key'] }}">
        <div class="mb-3 flex items-center justify-between">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-100">{{ $column['label'] }}</h2>
          <span class="rounded-full bg-white/10 px-2 py-1 text-xs font-semibold text-slate-200" data-count-for="{{ $column['key'] }}">0</span>
        </div>
        <div class="board-dropzone min-h-[18rem] space-y-3 rounded-2xl border border-dashed border-white/10 bg-black/10 p-2 transition-colors" data-dropzone="{{ $column['key'] }}">
